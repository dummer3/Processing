...
/*                                                                          */
/*          Exemple 5 : Realite Augmentee par ARtoolkit et Webcam           */
/*                      avec marqueur de type Pattern, avec Sarxos          */
/*                                                                          */
/* Exemple_5_RA_Pattern_Sarxos.pde                     Processing 3.5.4     */
/****************************************************************************/
...
void setup() {
  ...
  surface.setTitle("Exemple 5 - RA Pattern avec Sarxos - E. Mesnard / ISIMA");
  noFill();
  ...
  if (cameras.isEmpty()) {
    ...
    // Initialisation d' une webcam
    //webCam = Webcam.getDefault(); // Recuperation de la camera par defaut
    webCam = cameras.get(0); // recuperation de la webcam 1
...
void draw() {
  if (webCam.isImageNew() && webCam.isOpen()) { // Verification de presence d'une nouvelle frame
    ...
    PImgWebCam.updatePixels();    // Mise a jour des pixels
